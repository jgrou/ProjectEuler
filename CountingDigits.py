def count_nonzero_at_place(digit_to_count, limit, this_digit, this_place, base=10):
  assert digit_to_count > 0
...
def counts_at_each_place(digit_to_count, limit, base=10):
  limit_digits = strbase(limit, base)
  length = len(limit_digits)
  counter = count_nonzero_at_place
  return [counter(digit_to_count, limit, this_digit, length - i, base=base) for i, this_digit in enumerate(limit_digits)]

def count_by_digits(digit_to_count, limit, base=10):
  counts_by_digit = counts_at_each_place(digit_to_count, limit, base=base)
...
def unbounded_binary_ge_search(digit=1, base=10, safeleft=2, width=1):
...
for digit in range(1,10):
    ...
    while x is not None:
        s += x
        x = unbounded_binary_ge_search(digit=digit, safeleft=x)